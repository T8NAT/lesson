        ...
        <form id="kt_add_setting_form" action="{{ route('settings.store') }}" enctype="multipart/form-data" method="POST" class="form p-4" data-kt-redirect="{{route('settings.index')}}">
            ...
            <div class="row g-4 mt-2">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control" placeholder="اسم التطبيق">
                </div>
                <div class="col-md-6">
                    <label class="form-label">رابط التطبيق</label>
                    <input type="url" name="url" value="{{ old('url') }}" class="form-control" placeholder="رابط التطبيق">
                </div>
            </div>
            <div class="row g-4 mt-2">
                <div class="col-md-6">
                    <label class="form-label">الهاتف</label>
                    <input type="tel" name="phone" value="{{ old('phone') }}" class="form-control" placeholder="رقم الهاتف">
                </div>
                <div class="col-md-6">
            ...
            <div class="mt-3">
                <label class="form-label">وصف عن التطبيق</label>
                <textarea name="about" class="form-control" rows="4" placeholder="وصف الموقع">{{ old('about') }}</textarea>
            </div>
...
{{--                            <textarea name="meta_description" class="form-control" placeholder="وصف ميتا">{{ old('meta_description') }}</textarea>--}}
{{--                            <label class="form-label mt-2">الكلمات المفتاحية</label>--}}
{{--                            <textarea name="meta_keyword" class="form-control" placeholder="الكلمات المفتاحية">{{ old('meta_keyword') }}</textarea>--}}
{{--                            <label class="form-label mt-2">(Alt)نص بديل للصورة</label>--}}
{{--                            <textarea name="alt" class="form-control" placeholder="نص بديل للصورة">{{ old('alt') }}</textarea>--}}
{{--                        </div>--}}
{{--                    </div>--}}
{{--                </div>--}}
{{--            </div>--}}
